...
void setup() {
  ...
  // --- Test Move ---
  Serial.println("Moving pitch to 90 degrees...");
  float target = 90.0f;
  // This command just *schedules* the move. The ISR will do the work.
  axisPitch.moveToDeg(target); // Uses default speed/accel

  // Wait for the move to complete
  while (axisPitch.isMoving())
  {
    float pos = axisPitch.getPositionDeg();
    Serial.print("pitch pos: "); Serial.println(pos);
    delay(200);
  }
...
void loop() {
  ...
  const unsigned long pauseMs = 500; // 0.5 sec pause

  // We only schedule a new move IF the motor is not busy
  if (!axisPitch.isMoving()) {
    // Wait for a small pause before starting the next move
    if (millis() - lastChangeMs < pauseMs) {
      return; // Not time to move yet
    }

    // It's time! Schedule the next move.
    if (goingTo) {
      Serial.println("Loop: move to 140°");
      axisPitch.moveToDeg(140.0f); // Uses default speed/accel
    } else {
      Serial.println("Loop: move to 150°");
