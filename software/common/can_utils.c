...
bool CAN_Encode_Heartbeat(CAN_Frame_t *frame, uint32_t msg_id, const HeartbeatPayload_t *payload)
{
    if (!frame || !payload)
        return false;
    if (msg_id != CAN_ID_HB_VISION && msg_id != CAN_ID_HB_MOTION && msg_id != CAN_ID_HB_COORD)
        return false;
...
{
    if (!frame || !payload || frame->dlc != sizeof(HeartbeatPayload_t))
        return false;
    if (frame->id != CAN_ID_HB_VISION && frame->id != CAN_ID_HB_MOTION && frame->id != CAN_ID_HB_COORD)
        return false;
