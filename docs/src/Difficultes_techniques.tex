...
d'\textbf{acheter la carte du constructeur}. Le \emph{vision node} de VESPA ne l'a pas fait.
...
se relance 20\,362 fois. Le Jetson \emph{entend} parfaitement le STM32 ; il n'arrive pas à
lui \emph{répondre}.
...
\paragraph{L'\emph{overlay}, retrouvé et remis en place.} Le bon binaire a depuis été
récupéré : il est de nouveau dans \fichier{/boot}, référencé par la ligne \texttt{OVERLAYS}
de \fichier{/boot/extlinux/extlinux.conf} (modèle fourni :
\fichier{jetson/extlinux.conf.reference}), et sa copie de référence est versionnée dans
\fichier{jetson/dts/can0-pinmux.dtbo}, avec sa source \fichier{.dts}. Son en-tête interne
annonce « \texttt{CAN0 Hardware Override V5} » — cinquième itération. Il déclare les
n\oe uds corrects (\texttt{can0\_dout\_paa0}, \texttt{can0\_din\_paa1}, fonction
\texttt{can0}) et neutralise \texttt{paa2}/\texttt{paa3}. \textbf{C'est cette version qui a
produit les 111\,909 trames.}
...
état. Dans l'ordre :
\begin{enumerate}
  \item \textbf{Vérifier l'\emph{overlay}} : \fichier{can0-pinmux.dtbo} doit être dans
        \fichier{/boot} et sur la ligne \texttt{OVERLAYS} de \fichier{extlinux.conf} — il y
        est en l'état. S'il a disparu (mise à jour JetPack), le recopier depuis
        \fichier{jetson/dts/} et redémarrer.
...
Le bus a déjà fonctionné (111\,909 trames, 0 erreur) et l'\emph{overlay}
\fichier{can0-pinmux.dtbo} est en place dans \fichier{/boot}. Il ne reste qu'à remonter les
transceivers avec le correctif 5~V et à reconfirmer le sens émission.
\textbf{C'est une remise en état, pas une recherche.} \\
\addlinespace
